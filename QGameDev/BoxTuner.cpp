...
BoxTuner::BoxTuner(QWidget* parent, const QJsonObject& jobj)
	: Tuner(parent, jobj)
{
	m_ui.setupUi(this);
	...
	connect(m_ui.tuner_depth, SIGNAL(valueChanged(double)), this, SLOT(tuner_size_ValueChanged()));
	...
	if (att.contains("size"))
	{
		...
		m_ui.tuner_depth->setValue(values[2].toDouble());
	}
	...
	obj3d_tuner->m_ui.logo->setPixmap(QPixmap(QString(":/images/box.png")));
...
void BoxTuner::tuner3d_update(QJsonObject tuning)
{
...
void BoxTuner::tuner_material_update(QJsonObject tuning)
{
...
void BoxTuner::tuner_size_ValueChanged()
{
	if (!initialized) return;

	QJsonObject tuning;
	tuning["size"] = QString::number(m_ui.tuner_width->value()) + ", " + QString::number(m_ui.tuner_height->value()) + ", " + QString::number(m_ui.tuner_depth->value());
...
void BoxTuner::chk_is_building_Checked(bool checked)
{
	if (!initialized) return;
