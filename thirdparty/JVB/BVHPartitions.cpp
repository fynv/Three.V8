...
namespace flex_bvh
{
	...
	SpatialSplit partition_spatial(const std::vector<Triangle>& triangles, const std::vector<PrimitiveRef> indices[3], int first_index, int index_count, float* sah, AABB bounds)
	{
		...
		for (int dimension = 0; dimension < 3; dimension++)
		{
			float bounds_min = bounds.pos_min[dimension] - 0.001f;
			float bounds_max = bounds.pos_max[dimension] + 0.001f;
			...
			for (int i = first_index; i < first_index + index_count; i++)
			{
				...
				if (vertices[0][dimension] > vertices[1][dimension]) swap(vertices[0], vertices[1]);
				if (vertices[1][dimension] > vertices[2][dimension]) swap(vertices[1], vertices[2]);
				if (vertices[0][dimension] > vertices[1][dimension]) swap(vertices[0], vertices[1]);

				float triangle_aabb_min = triangle_aabb.pos_min[dimension];
				float triangle_aabb_max = triangle_aabb.pos_max[dimension];
				...
				bin_min = glm::clamp(bin_min, 0, SBVH_BIN_COUNT - 1);
				bin_max = glm::clamp(bin_max, 0, SBVH_BIN_COUNT - 1);
