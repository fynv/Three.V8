...
namespace flex_bvh 
{
	template<typename T, typename Cmp = LessThan<T>>
	...
	constexpr void quick_sort(T * first, T * last, Cmp cmp = { }) {
		...
		if (cmp(*three[1], *three[0])) swap(three[0], three[1]);
		if (cmp(*three[2], *three[1])) swap(three[1], three[2]);
		if (cmp(*three[1], *three[0])) swap(three[0], three[1]);
	...
	struct RadixSortAdapter<float> {
		FORCE_INLINE unsigned operator()(const float & x) const {
			unsigned y = bit_cast<unsigned>(x);
	...
	void radix_sort(T * first, T * last, Adapter adapter = { }) {
		std::vector<T> tmp = std::vector<T>(last - first);
