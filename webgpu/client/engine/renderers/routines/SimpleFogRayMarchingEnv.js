...
function GetPipelineSimpleFogRayMarchingEnv(options)
{
    let signature = JSON.stringify(options);
    if (!("fog_ray_marching_env_simple" in engine_ctx.cache.pipelines))
    {
        engine_ctx.cache.pipelines.fog_ray_marching_env_simple = {};
    }

    if (!(signature in engine_ctx.cache.pipelines.fog_ray_marching_env_simple))
    {
        ...
        engine_ctx.cache.pipelines.fog_ray_marching_env_simple[signature] =  engine_ctx.device.createRenderPipeline(pipelineDesc);
    }

    return engine_ctx.cache.pipelines.fog_ray_marching_env_simple[signature];
}

export function SimpleFogRayMarchingEnv(passEncoder, params)
{
    let options = {}; 
    ...
    let pipeline = GetPipelineSimpleFogRayMarchingEnv(options);
