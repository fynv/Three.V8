...
function get_shader(options)
{
    ...
    let has_uv = mOpt.has_color_texture || mOpt.has_metalness_map || mOpt.has_specular_map || mOpt.has_emissive_map;
...
function GetPipelineSimple(options)
{
    let signature = JSON.stringify(options);
    if (!("simple" in engine_ctx.cache.pipelines))
    {
        engine_ctx.cache.pipelines.simple = {};
    }
    ...
    if (!(signature in engine_ctx.cache.pipelines.simple))
    {
        ...
        engine_ctx.cache.pipelines.simple[signature] = engine_ctx.device.createRenderPipeline(pipelineDesc); 
    }
    return engine_ctx.cache.pipelines.simple[signature];
}

export function RenderSimple(passEncoder, params)
{
...
export function RenderSimpleBundle(params)
{
    let primitive = params.primitive;   
    let material = params.material_list[primitive.material_idx];
    ...
    RenderSimple(renderBundleEncoder, params);
