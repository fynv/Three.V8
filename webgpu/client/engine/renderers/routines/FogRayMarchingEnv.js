...
fn getIrradiance(world_pos: vec3f) -> vec3f
{
    ...
    for (var i=0; i<8; i++)
    {
        ...
        var weight = get_visibility(world_pos, vert, vert_world);	
...
`)}
    ...
    var step = dis/f32(uFog.max_num_steps)*4.0;
    if (step<uFog.min_step*4.0) 
    {
        step = uFog.min_step*4.0;
    }
...
function GetPipelineFogRayMarchingEnv(options)
{
    let signature = JSON.stringify(options);
    if (!("fog_ray_marching_env" in engine_ctx.cache.pipelines))
    {
        engine_ctx.cache.pipelines.fog_ray_marching_env = {};
    }

    if (!(signature in engine_ctx.cache.pipelines.fog_ray_marching_env))
    {
        ...
        let bindGroupLayouts = [
            ...
            engine_ctx.cache.bindGroupLayouts.fog_probe_grid
        ];
        ...
        engine_ctx.cache.pipelines.fog_ray_marching_env[signature] =  engine_ctx.device.createRenderPipeline(pipelineDesc);
    }

    return engine_ctx.cache.pipelines.fog_ray_marching_env[signature];
}

export function FogRayMarchingEnv(passEncoder, params)
{
    let options = {}; 
    ...
    let pipeline = GetPipelineFogRayMarchingEnv(options);
