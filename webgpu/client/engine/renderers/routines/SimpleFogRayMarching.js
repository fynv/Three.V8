...
`)}
    ...
    var step = dis/f32(uFog.max_num_steps)*2.0;
    if (step<uFog.min_step*2.0) 
    {
        step = uFog.min_step*2.0;
    }
...
`)}
        ...
        var step = (end - start)/f32(uFog.max_num_steps)*2.0;
        if (step<uFog.min_step*2.0) 
        {
            step = uFog.min_step*2.0;
        }
...
function GetPipelineSimpleFogRayMarching(options)
{
    let signature = JSON.stringify(options);
    if (!("fog_ray_marching_simple" in engine_ctx.cache.pipelines))
    {
        engine_ctx.cache.pipelines.fog_ray_marching_simple = {};
    }

    if (!(signature in engine_ctx.cache.pipelines.fog_ray_marching_simple))
    {
        ...
        engine_ctx.cache.pipelines.fog_ray_marching_simple[signature] =  engine_ctx.device.createRenderPipeline(pipelineDesc);

    }

    return engine_ctx.cache.pipelines.fog_ray_marching_simple[signature];
}


export function SimpleFogRayMarching(passEncoder, params)
{
    ...
    {        
        ...
        let pipeline = GetPipelineSimpleFogRayMarching(options);
