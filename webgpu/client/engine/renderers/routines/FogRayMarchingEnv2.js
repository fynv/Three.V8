...
function GetPipelineFogRayMarchingEnv2(options)
{
    let signature = JSON.stringify(options);
    if (!("fog_ray_marching_env2" in engine_ctx.cache.pipelines))
    {
        engine_ctx.cache.pipelines.fog_ray_marching_env2 = {};
    }

    if (!(signature in engine_ctx.cache.pipelines.fog_ray_marching_env2))
    {
        ...
        engine_ctx.cache.pipelines.fog_ray_marching_env2[signature] =  engine_ctx.device.createRenderPipeline(pipelineDesc);
    }

    return engine_ctx.cache.pipelines.fog_ray_marching_env2[signature];
}

export function FogRayMarchingEnv2(passEncoder, params)
{
    let options = {}; 
    options.is_msaa  = params.target.msaa;
    options.view_format= params.target.view_format;

    let pipeline = GetPipelineFogRayMarchingEnv2(options);
