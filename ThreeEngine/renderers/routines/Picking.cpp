...
layout (location = LOCATION_IDX) uniform uint uIdx;

layout (location = 0) out uvec4 out0;
...
#endif
	out0 = uvec4(uIdx, uIdx, uIdx, uIdx);
}
)";
...
void Picking::s_generate_shaders(const Options& options, Bindings& bindings, std::string& s_vertex, std::string& s_frag)
{
	s_vertex = g_vertex;
	s_frag = g_frag;
	...
	{
		bindings.location_idx = bindings.location_tex_color + 1;

		{
			char line[64];
			sprintf(line, "#define LOCATION_IDX %d\n", bindings.location_idx);
			defines += line;
		}
	}
...
Picking::Picking(const Options& options) : m_options(options)
{
...
void Picking::render(const RenderParams& params)
{
	...
	glUniform1ui(m_bindings.location_idx, params.idx);
