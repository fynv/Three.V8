...
#endif
	vec3 col = clamp(base_color.xyz, 0.0, 1.0);	
	out0 = vec4(col, 1.0);
}
)";
...
void BaseColorRoutine::s_generate_shaders(const Options& options, Bindings& bindings, std::string& s_vertex, std::string& s_frag)
{
	s_vertex = g_vertex;
	s_frag = g_frag;
	...
	if (options.has_color)
	{
		...
		bindings.location_varying_color = 0;
	...
	else
	{
		...
		bindings.location_varying_color = -1;
	}

	bool has_uv = options.has_color_texture;
...
BaseColorRoutine::BaseColorRoutine(const Options& options) : m_options(options)
{
...
void BaseColorRoutine::render(const RenderParams& params)
{
	...
	bool has_uv = m_options.has_color_texture;
	if (has_uv)
	{
