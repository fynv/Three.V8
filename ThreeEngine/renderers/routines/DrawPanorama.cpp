...
layout (location = 0) uniform sampler2D uTexSky;
...
void main()
{
    ...
    vec2 uv = vec2(0.5 + beta / (PI * 2.0), 0.5 - alpha / PI);
    outColor = texture(uTexSky, uv);
}
)";

DrawPanorama::DrawPanorama()
{
...
void DrawPanorama::render(const GLBuffer* constant_camera, const GLTexture2D* tex_sky)
{
    ...
    glBindTexture(GL_TEXTURE_2D, tex_sky->tex_id);
    glUniform1i(0, 0);
