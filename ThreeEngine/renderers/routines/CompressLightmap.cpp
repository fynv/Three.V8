...
layout (location = 3) uniform sampler2D uTexLightmap;

layout (binding=0, r8ui) uniform uimage2D uImgProbeVis;
...
void main()
{
	...
	vec3 lightmap_irr = texelFetch(uTexLightmap, texel_coord, 0).xyz * PI;
	...
	for (int i=0; i<8; i++)
	{
		...
		irrs[i] = get_irradiance_common(idx_probe, sample_dir);
		irr += irrs[i] * weight;
	}
...
CompressLightmap::CompressLightmap(bool has_probe_grid, bool has_lod_probe_grid)
...
void CompressLightmap::compress(const RenderParams& params)
{
	...
	glBindTexture(GL_TEXTURE_2D, params.light_map->tex_id);
	glUniform1i(3, 3);

	glBindImageTexture(0, params.probe_visibility_map->tex_id, 0, GL_TRUE, 0, GL_WRITE_ONLY, GL_R8UI);
