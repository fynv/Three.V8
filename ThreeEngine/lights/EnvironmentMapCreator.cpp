...
void EnvironmentMapCreator::Create(int width, int height, const GLCubemap * cubemap, EnvironmentMap * envMap)
{
	...
	for (int i = 0; i < 6; i++)
	{
		...
		{
			const uint8_t* pixel = &face[(size_t)j * 4];
			glm::vec3 color = { (float)pixel[0] / 255.0f, (float)pixel[1] / 255.0f, (float)pixel[2] / 255.0f };
			...
			for (int k = 0; k < 9; k++)
			{
				envMap->shCoefficients[k] += glm::vec4(shBasis[k] * weight * color, 0.0f);
			}
		}
	}
