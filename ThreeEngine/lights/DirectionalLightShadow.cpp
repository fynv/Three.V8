	...
	, constant_shadow(sizeof(ConstDirectionalShadow), GL_UNIFORM_BUFFER)
{
	...
	{
		glGenFramebuffers(1, &m_lightFBO0);
		glBindFramebuffer(GL_FRAMEBUFFER, m_lightFBO0);

		glGenTextures(1, &m_lightTex0);
		glBindTexture(GL_TEXTURE_2D, m_lightTex0);
		...
		glFramebufferTexture(GL_FRAMEBUFFER, GL_DEPTH_ATTACHMENT, m_lightTex0, 0);
...
DirectionalLightShadow::~DirectionalLightShadow()
{
	...
	glDeleteTextures(1, &m_lightTex0);
	glDeleteFramebuffers(1, &m_lightFBO0);
}
